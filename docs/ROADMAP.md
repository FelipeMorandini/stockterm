...
|---|-------|------------------|
| [#199](https://github.com/FelipeMorandini/stockterm/issues/199) | Charts: precompute candle layout off render path | §63 follow-up — cache `layout_candles` result on `App`, invalidate on viewport/bar/range/area changes — **§64** (shipped **2026-05-25**). |
| [#200](https://github.com/FelipeMorandini/stockterm/issues/200) | HistoricalData: normalize bar timestamp unit at ingest | §63 follow-up — Rustdoc `t` = Unix ms; `normalize_bar_timestamp_to_ms` at provider write sites; remove `TIMESTAMP_MS_EPOCH_THRESHOLD` — **§65** (shipped **2026-05-25**). |
