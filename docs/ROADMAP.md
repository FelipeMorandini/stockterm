...
|---|-------|------------------|
| [#190](https://github.com/FelipeMorandini/stockterm/issues/190) | Charts: candlestick visual density polish | Terminal candle layout vs web charts — §4.4 / §5 item 1. |
| [#192](https://github.com/FelipeMorandini/stockterm/issues/192) | Config::save: stop dropping I/O errors | Migrate legacy `save()` callers to `try_save` + log/surface — §4.15. |

### Optional / deferred

| # | Title | Scope (one line) |
|---|-------|------------------|
| [#191](https://github.com/FelipeMorandini/stockterm/issues/191) | Optional CancellationToken for quote batches | Only if overlapping batches become a product requirement — §16.1. |
...
| [#195](https://github.com/FelipeMorandini/stockterm/issues/195) | Options: lazy `options_theme_stamp` compare | §56.7 optimization — skip redundant rebuilds. |
| [#196](https://github.com/FelipeMorandini/stockterm/issues/196) | Theme: watchlist/portfolio/alerts cache invalidation | §56.7 — restyle on Settings commit beyond Options tab. |
| [#79](https://github.com/FelipeMorandini/stockterm/issues/79)  | Unicode / full case-folding for ticker normalization | Deferred — ASCII-only covers shipped providers. |
...
- **Partial — further polish** — [#190](https://github.com/FelipeMorandini/stockterm/issues/190) dense candle layout vs web charts; [#79](https://github.com/FelipeMorandini/stockterm/issues/79) Unicode tickers (deferred).
...
- **Partial — §16 slice shipped (Issues [#17](https://github.com/FelipeMorandini/stockterm/issues/17) / [#46](https://github.com/FelipeMorandini/stockterm/issues/46) / [#77](https://github.com/FelipeMorandini/stockterm/issues/77); [`docs/SPEC.md`](SPEC.md) §16.8); optional cancel token remains**
  ...
  - **Shipped (§40 / 2026-05-19):** [#56](https://github.com/FelipeMorandini/stockterm/issues/56) explicit **`Semaphore::acquire`** handling in quote fan-out — see [`docs/SPEC.md`](SPEC.md) §40.2. **Optional follow-up:** [#191](https://github.com/FelipeMorandini/stockterm/issues/191) **`CancellationToken`** if overlapping batches are introduced.
...
- **Implemented**
  ...
  - **Open follow-up:** [#192](https://github.com/FelipeMorandini/stockterm/issues/192) — `Config::save` still discards I/O errors via `let _ = self.try_save()`; hot paths already use `try_save_config_with_session`.
...
1. **Charts polish (low priority):** [#190](https://github.com/FelipeMorandini/stockterm/issues/190) — visual density vs web charts ([§4.4](#44-core--historical-charts-in-terminal)).
2. **CI (M7):** **Shipped** — `.github/workflows/ci.yml` + `insta` error-overlay snapshots (**§57–§58**, Issues [#181](https://github.com/FelipeMorandini/stockterm/issues/181), [#184](https://github.com/FelipeMorandini/stockterm/issues/184); **PR:** [#188](https://github.com/FelipeMorandini/stockterm/pull/188)). **Shipped — §59 / [#189](https://github.com/FelipeMorandini/stockterm/issues/189):** expanded UI snapshots (status bar, portfolio dialog, Options). **Shipped — §60 / [#193](https://github.com/FelipeMorandini/stockterm/issues/193):** blocking `cargo fmt --check` on both CI jobs.
3. **Optional:** [#191](https://github.com/FelipeMorandini/stockterm/issues/191) **`CancellationToken`** — [`docs/SPEC.md`](SPEC.md) §16.1 item 2.
4. **Optional:** [#192](https://github.com/FelipeMorandini/stockterm/issues/192) `Config::save` I/O errors — [§4.15](#415-technical--config-file-for-prefs--portfolio).
5. **Optional:** [#194](https://github.com/FelipeMorandini/stockterm/issues/194) saved/regex filters — M6 follow-on beyond §23.
6. **Optional:** [#195](https://github.com/FelipeMorandini/stockterm/issues/195) Options lazy theme stamp — SPEC §56.7.
7. **Optional:** [#196](https://github.com/FelipeMorandini/stockterm/issues/196) watchlist/portfolio/alerts theme cache invalidation — SPEC §56.7.
...
8. **M7 — Tests & CI** ✅ **Largely delivered**
   ...
   - **Follow-on:** [#190](https://github.com/FelipeMorandini/stockterm/issues/190) (charts polish). **Shipped — §59 / [#189](https://github.com/FelipeMorandini/stockterm/issues/189):** expand snapshots. **Shipped — §60 / [#193](https://github.com/FelipeMorandini/stockterm/issues/193):** blocking fmt.
