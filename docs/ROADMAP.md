...
All currently **OPEN** `roadmap`-labelled issues on `FelipeMorandini/stockterm`, verified 2026-05-23. Closed issues are intentionally omitted — see commit history / per-section `Shipped` lines for what has landed.
...
1. **Charts polish (low priority):** core charts (zoom, pan, time ranges, candlestick widget, indicators, partial-page messaging, `time_range` / `chart_mode` persistence per §54) are implemented through §52 / §53 / §54; remaining work is mainly the visual density gap vs web charts ([§4.4](#44-core--historical-charts-in-terminal)).
