...
- **Implemented — [#136](https://github.com/FelipeMorandini/stockterm/issues/136) / [`docs/SPEC.md`](SPEC.md) §26:** Keymap phase 2 — explicit default chords for portfolio / alert dialog digits and Settings edit buffer (`PortfolioDialogDigitOrDot`, `AlertDialogDigitOrDot`, `SettingsEditDigit`, `SettingsEditSymbolChar`); Settings edit **Shift+letter** fallback when no chord matches; Stock View symbol + Search query + alert dialog letters / condition **`a`/`b`** remain §24.5 / §26 wildcards; **manual QA** [`docs/QA_PLAN.md`](QA_PLAN.md) Issue **#136** (sign-off pending).
