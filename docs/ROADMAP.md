...
2. **CI (M7):** **Shipped in-tree** — `.github/workflows/ci.yml` runs `cargo clippy` + `cargo test` on PRs and `main` (default + `--no-default-features` jobs); **`insta`** snapshots for `draw_error_log_overlay` (**§57–§58**, Issues [#181](https://github.com/FelipeMorandini/stockterm/issues/181), [#184](https://github.com/FelipeMorandini/stockterm/issues/184)). Manual QA sign-off pending. Follow-on: more `draw_*` snapshots (Stock View status, portfolio dialog — see SPEC §58.6).
