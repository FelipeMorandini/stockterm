...
2. **CI (M7):** **Shipped** — `.github/workflows/ci.yml` runs `cargo clippy` + `cargo test` on PRs and `main` (default + `--no-default-features` jobs); **`insta`** snapshots for `draw_error_log_overlay` (**§57–§58**, Issues [#181](https://github.com/FelipeMorandini/stockterm/issues/181), [#184](https://github.com/FelipeMorandini/stockterm/issues/184); QA sign-off **2026-05-23**, **PR:** [#188](https://github.com/FelipeMorandini/stockterm/pull/188)). Follow-on: more `draw_*` snapshots (Stock View status, portfolio dialog — see SPEC §58.6).
