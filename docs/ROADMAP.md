...
- **Follow-up (product / §24):** User JSON overlays assign each [`Action`](../src/config/keymap.rs) to a single canonical [`BindingLayer`](../src/config/keymap.rs) via `action_binding_layer`, while built-in defaults may register the same action in more than one layer (for example portfolio list vs remove-confirm armed). Row-key remaps for the main table may not apply in armed mode until the model supports per-context actions or context-aware overlay merge — tracked in **[#134](https://github.com/FelipeMorandini/stockterm/issues/134)**.
- **Partial — symbol buffer + modal field typing**
  - Stock View / Search query / Settings edit buffers and portfolio + alert dialog digit/symbol entry still use literal **`KeyCode::Char`** arms after keymap resolution (per SPEC §24.5 exception).
