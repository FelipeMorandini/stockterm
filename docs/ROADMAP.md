...
All currently **OPEN** `roadmap`-labelled issues on `FelipeMorandini/stockterm`, verified 2026-05-22. Closed issues are intentionally omitted — see commit history / per-section `Shipped` lines for what has landed.

| # | Title | Scope (one line) |
|---|-------|------------------|
| [#180](https://github.com/FelipeMorandini/stockterm/issues/180) | Charts: persist `time_range` and `chart_mode` in `~/.stockterm.json` | Restore selected `TimeRange` (1D/1W/1M/1Y) and line/candlestick mode across launches (SPEC §11 / §22.7 follow-up). |
| [#181](https://github.com/FelipeMorandini/stockterm/issues/181) | M7: GitHub Actions CI (`cargo test` + `clippy -D warnings`) | Wire `.github/workflows/ci.yml` so the per-issue QA matrix runs on every PR. |
| [#182](https://github.com/FelipeMorandini/stockterm/issues/182) | Portfolio: row edit UI for existing holdings | Inline edit for `shares` / `avg_cost` on a selected holding (no remove + re-add). |
| [#183](https://github.com/FelipeMorandini/stockterm/issues/183) | Options: rebuild display tables when theme preset changes | `settings_commit_theme_preset` must invalidate `OptionsDisplayCache` so §53.2 zero-clone tables repaint. |
| [#184](https://github.com/FelipeMorandini/stockterm/issues/184) | M7: `TestBackend` draw snapshot tests (error overlay first) | First snapshot test on `draw_error_log_overlay` (SPEC §20.15); gated by [#181](https://github.com/FelipeMorandini/stockterm/issues/181). |
| [#79](https://github.com/FelipeMorandini/stockterm/issues/79)  | Unicode / full case-folding for ticker normalization | Deferred — current ASCII-only `to_ascii_uppercase` covers shipped providers. |
| [#68](https://github.com/FelipeMorandini/stockterm/issues/68)  | Portfolio: optional fixed-point / decimal types for money and shares | Optional precision upgrade; current `f64` matches Yahoo/Polygon precision. |
| [#24](https://github.com/FelipeMorandini/stockterm/issues/24)  | Advanced: custom widgets — post-MVP | Deferred until a concrete widget need outgrows ratatui built-ins. |

**Triage policy:** [#24](https://github.com/FelipeMorandini/stockterm/issues/24), [#68](https://github.com/FelipeMorandini/stockterm/issues/68), [#79](https://github.com/FelipeMorandini/stockterm/issues/79) are intentionally deferred and should not be auto-closed during backlog sweeps.
...
- **Open follow-ups:** optional decimal money ([#68](https://github.com/FelipeMorandini/stockterm/issues/68), deferred) and row edit UI for existing holdings ([#182](https://github.com/FelipeMorandini/stockterm/issues/182), filed 2026-05-22 — see §2.1).
...
- **Partial — further polish** — dense candle layout vs web charts; [#79](https://github.com/FelipeMorandini/stockterm/issues/79) Unicode tickers. **Shipped — §40:** [#36](https://github.com/FelipeMorandini/stockterm/issues/36) chart timestamp hardening — see [`docs/SPEC.md`](SPEC.md) §40.1. **Shipped — §39:** [#78](https://github.com/FelipeMorandini/stockterm/issues/78) / [#87](https://github.com/FelipeMorandini/stockterm/issues/87) / [#108](https://github.com/FelipeMorandini/stockterm/issues/108) — see [`docs/SPEC.md`](SPEC.md) §39.10.
...
- **Partial — §16 slice shipped (Issues [#17](https://github.com/FelipeMorandini/stockterm/issues/17) / [#46](https://github.com/FelipeMorandini/stockterm/issues/46) / [#77](https://github.com/FelipeMorandini/stockterm/issues/77); [`docs/SPEC.md`](SPEC.md) §16.8); optional cancel token remains**
  ...
  - **Shipped (2026-05-11):** **`STOCKTERM_DEBUG_HTTP_DELAY_MS`** before quote fan-out; stock batch **`catch_unwind`** + synthetic `FetchDone::Stock` on panic; **`apply_inflight_recovery(Stock)`** drains **`stock_refresh_pending`**; recovery **`send`** failures logged. **Shipped (§38 / 2026-05-18):** [#76](https://github.com/FelipeMorandini/stockterm/issues/76) tracing fetch drops, [#85](https://github.com/FelipeMorandini/stockterm/issues/85) delay cap, [#86](https://github.com/FelipeMorandini/stockterm/issues/86) dev panic log — see [`docs/SPEC.md`](SPEC.md) §38.12. **Shipped (§39 / 2026-05-19):** [#108](https://github.com/FelipeMorandini/stockterm/issues/108) event-thread join on exit, [#78](https://github.com/FelipeMorandini/stockterm/issues/78) stale-inflight watchdog + **`deliver_fetch_done`**, [#87](https://github.com/FelipeMorandini/stockterm/issues/87) documented unbounded **`mpsc`** policy — see [`docs/SPEC.md`](SPEC.md) §39.10. **Shipped (§40 / 2026-05-19):** [#56](https://github.com/FelipeMorandini/stockterm/issues/56) explicit **`Semaphore::acquire`** handling in quote fan-out — see [`docs/SPEC.md`](SPEC.md) §40.2. **Optional follow-up:** **`CancellationToken`** if overlapping batches are introduced; clippy lock hygiene on future refactors.
...
- **Implemented**
  ...
  - **Remaining gap:** `Config::save` still discards I/O errors via `let _ = self.try_save()` (`src/config/config.rs:175`). Hot paths already use `try_save_config_with_session` and surface failures inline, so the fallback `save()` is mostly compatibility — the bare `save()` callers should migrate to `try_save` with a logged error.
...
- **Follow-up — open issue:** Charts `time_range` and `chart_mode` do **not** yet persist — see [#180](https://github.com/FelipeMorandini/stockterm/issues/180) (filed 2026-05-22, SPEC §11 line ~955 follow-up). Planned for the next SPEC slice.
...
1. **Charts polish (low priority):** core charts (zoom, pan, time ranges, candlestick widget, indicators, partial-page messaging) are implemented through §52 / §53; remaining work is the visual density gap vs web charts ([§4.4](#44-core--historical-charts-in-terminal)) and per-session state persistence ([#180](https://github.com/FelipeMorandini/stockterm/issues/180)).
2. **CI gap (M7):** the test suite is **not** thin — `cargo test --lib` runs ~**302** unit tests across `src/`, plus `wiremock` integration coverage. The real gap is that **no `.github/workflows/*.yml` exists**, so this matrix runs only on the maintainer's machine. Tracked in [#181](https://github.com/FelipeMorandini/stockterm/issues/181) (CI) and [#184](https://github.com/FelipeMorandini/stockterm/issues/184) (`TestBackend` snapshot tests, gated by [#181](https://github.com/FelipeMorandini/stockterm/issues/181)).
3. **Optional:** **`CancellationToken`** for quote overlap — [`docs/SPEC.md`](SPEC.md) §16.1 item 2 (if product adds overlapping batches).
4. **Optional:** `Config::save` still drops I/O errors (`src/config/config.rs:175`) — see [§4.15](#415-technical--config-file-for-prefs--portfolio); cosmetic only because hot paths use `try_save_config_with_session`.
...
> **Status (2026-05-22):** M0 – M8 are largely delivered (Yahoo + Polygon, watchlist, portfolio, charts, alerts, search/news/settings, theme, layout, filters, keymap, crypto, indicators, backtesting, options). The current actionable backlog is in [§2.1 Active backlog (GitHub)](#21-active-backlog-github) — the milestone list below is preserved for historical context.
