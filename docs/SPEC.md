...
**Status:** **Phase A shipped** (2026-05-26) — `Tab::Dashboard`, config-driven grid, read-only **watchlist** panes, `dual_watchlist` fixture. Phases B–D (additional pane kinds, in-app editor) remain future work. **Manual QA:** [`docs/QA_PLAN.md`](QA_PLAN.md) Issue **#24** (sign-off pending; **PR:** [#207](https://github.com/FelipeMorandini/stockterm/pull/207)).
...
### 70.1 Problem (verified in tree)

| Area | Location | Gap |
|------|----------|-----|
| Tab model | [`Tab`](../src/app/app.rs) | Fixed tabs (`StockView`, `Portfolio`, …); no **Dashboard** tab. |
| Layout | [`config/layout.rs`](../src/config/layout.rs) | Preset-driven splits for **one** tab at a time; no N-pane grid from JSON. |
| Draw paths | [`ui.rs`](../src/app/ui.rs), [`portfolio.rs`](../src/app/portfolio.rs), [`charts.rs`](../src/app/charts.rs) | Pane functions assume full tab body or hard-coded two-pane splits (`draw_stock_view`). |
| Config | [`config/config.rs`](../src/config/config.rs) | No `dashboards` / `active_dashboard` fields. |
| Custom ratatui widgets | [`charts.rs`](../src/app/charts.rs) `CandlestickChart` | One bespoke **`impl Widget`** for candles only; not a general dashboard framework. |

**Conclusion:** Power users cannot compose multiple data views in one screen without duplicating tab switches. Existing tabs already cover each data source in isolation; **§70** adds **composition**, not new market-data APIs.
...
- Config add/remove/reorder panes takes effect on **next launch** (v1 is file-edited; see §70.9).
- Each pane **degrades gracefully** when data is empty or errored (inline placeholder; no panic).

**Non-goals (v1):**

- In-app dashboard editor (drag-resize, add-pane wizard) — future issue.
...
| Phase | Deliverable |
|-------|-------------|
| **A (MVP — Issue #24 AC)** | Models + config + `Tab::Dashboard` + `Watchlist` pane ×2 preset + degradation |
| **B** | `StockDetail`, `News`, `Portfolio`, `AlertsList` panes via refactored helpers |
| **C** | `Chart` + `IndicatorSummary` panes (coordinate with **§46** / **§64**) |
| **D** | In-app editor + live preview (new GitHub issue) |

Engineer ships **Phase A** first; Phases B–C may land in the same PR only if refactors stay small.
...
- [x] **`cargo test`** + **`cargo clippy -- -D warnings`** green.
- [x] Manual QA Issue **#24** signed.
- [x] No **`println!`** / UI-thread HTTP; logging via **`tracing`** only.

### 70.13 Out of scope

- Drag-and-drop dashboard editor; live resize without restart (v1).
...
After maintainer approval of **§70** **and** a **§70.2** product trigger is met, the **engineer** may implement per [`.cursor/rules/sdd_workflow.mdc`](../.cursor/rules/sdd_workflow.mdc) and run [`docs/QA_PLAN.md`](QA_PLAN.md) Issue **#24** before merge. If the trigger is **not** met, keep issue **#24** open per ROADMAP triage policy (do not auto-close).