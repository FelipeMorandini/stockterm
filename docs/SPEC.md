...
- [#79](https://github.com/FelipeMorandini/stockterm/issues/79) Unicode ticker normalization.
- **`CancellationToken`** for overlapping HTTP batches (§16 optional).
...
- Lazy stamp compare in `draw_options` (`options_theme_stamp` vs palette) — optional future optimization; **not** required for #183.
- Rebuilding watchlist/portfolio/alerts draw caches on theme change (separate issues; those paths mostly borrow live `rt` today).
...
| **`cargo fmt --check` advisory** | Issue #181 allows non-blocking fmt until the tree is consistently formatted; `continue-on-error: true`. |
...
- [ ] PR UI shows a required (or de-facto required) status check from the workflow.
...
- [ ] Manual QA [`docs/QA_PLAN.md`](QA_PLAN.md) Issue **#181** signed.
...
- **Status:** Shipped in-tree (manual QA [`docs/QA_PLAN.md`](QA_PLAN.md) Issue **#181** pending; closes with **§58** / Issue **#184**).
- **Tracking:** [Issue #181](https://github.com/FelipeMorandini/stockterm/issues/181).
- **Code:** `.github/workflows/ci.yml`, `README.md`.
- **Blocks:** None (unblocks **§58**).
...
### 58.6 Future snapshot targets (out of scope for #184)

Document for follow-up issues (not #184):

| Target | SPEC / issue |
|--------|----------------|
| Stock View narrow status | §37.1 / Issue **#81** |
| Portfolio add dialog | §18.13 / Issue **#93** |
| `draw_options` full tab | §52.2 — larger surface; consider after overlay pattern proven |
...
- [ ] Manual QA [`docs/QA_PLAN.md`](QA_PLAN.md) Issue **#184** signed.
...
- **Status:** Shipped in-tree (manual QA [`docs/QA_PLAN.md`](QA_PLAN.md) Issue **#184** pending; same PR as **§57** / Issue **#181**).
