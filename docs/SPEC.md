...
3. **Stale watchdog** — **`fn recover_stale_inflight_flags(&mut self)`** called from **`on_background_tick`** (after TTL/session flush):
   ...
   - **Stock (overlap — §74 / [#214](https://github.com/FelipeMorandini/stockterm/issues/214)):** When **`allow_overlapping_quote_batches`** is **`true`**, superseding spawns reset **`stock_inflight_since`** but not **`stock_inflight_chain_since`**. The stock watchdog must use the **chain** timestamp so repeated supersede cannot defer recovery indefinitely. See **§74.3**.
...
- [#191](https://github.com/FelipeMorandini/stockterm/issues/191) **`CancellationToken`** for overlapping HTTP batches — **§68** (**shipped** 2026-05-30; was out of scope for §39).
...
**Status:** **Shipped** (2026-05-30) — **`tokio-util`** cancellation, **`allow_overlapping_quote_batches`** (default **`false`**), stale-generation tests; manual QA [`docs/QA_PLAN.md`](QA_PLAN.md) Issue **#191** sign-off **2026-05-30** (**PR:** [#215](https://github.com/FelipeMorandini/stockterm/pull/215)).

**GitHub issue (synced 2026-05-30):** OPEN · label **`roadmap`** · title *Optional: CancellationToken for superseded quote batches (§16.1)*. Acceptance: SPEC §16.1 when-cancel docs, superseded-batch test, clippy clean, no UI blocking. **Out of scope:** overlap solely to exercise cancellation.
...
- **§39** — **`deliver_fetch_done`** + **`InflightRecovery::Stock`** when **`FetchDone`** channel send fails; re-audit when cancelled tasks stop delivering (**§68.4.6**).
...
### 68.1 Problem (verified in tree, 2026-05-30)

| Mechanism | Location (approx.) | Behavior |
|-----------|-------------------|----------|
| **Single-flight guard** | [`request_immediate_stock_poll`](../src/app/app.rs) (~L1445), [`try_spawn_stock_poll_throttled`](../src/app/app.rs) (~L1453) | If **`stock_refresh_inflight`**, do **not** call **`spawn_stock_fetch_task`**; immediate poll sets **`stock_refresh_pending`** only. |
| **Batch worker** | [`run_stock_quote_batch`](../src/app/app.rs) (~L534) | Yahoo: **`yahoo_latest_quotes_for_symbols`** (chunk + fallback **`JoinSet`**). Polygon: per-symbol **`JoinSet`** + **`Semaphore`**. No cancel parameter today. |
| **Generation bump** | [`spawn_stock_fetch_task`](../src/app/app.rs) (~L1467) | **`stock_fetch_generation += 1`** then **`tokio::spawn`** + **`catch_unwind`** (**§16.2**). |
| **Stale result drop** | [`apply_stock_fetch_done`](../src/app/app.rs) (~L1521) | **`generation != stock_fetch_generation`** → early return; **does not** clear **`stock_refresh_inflight`** (**§16.2.1**). |
| **Coalesced follow-up** | [`apply_stock_fetch_done`](../src/app/app.rs) tail (~L1622), [`apply_inflight_recovery(Stock)`](../src/app/app.rs) (~L2598) | **`stock_refresh_pending`** → **`spawn_stock_fetch_task`** after apply or recovery (**§16.3**). |
| **Delivery** | [`deliver_fetch_done`](../src/app/fetch_delivery.rs) | **`FetchDone`** on UI channel, else **`InflightRecovery`** (**§39**). |

**Dependency gap:** [`Cargo.toml`](../Cargo.toml) has **no** **`tokio-util`** — add only for §68 mode **B**.
...
3. **Polygon fan-out:** in the **`JoinSet`** loop, **`tokio::select!`** **`biased;`** **`cancel.cancelled()`** vs **`joined`** — stop spawning new symbols when cancelled; prefer draining in-flight permits rather than **`JoinSet::abort_all`** unless join errors require it.

4. **Yahoo path (requires API-layer hooks):** [`yahoo_latest_quotes_for_symbols`](../src/api/yahoo.rs) (~L340) runs a **sequential** **`v7`** chunk loop then a fallback **`JoinSet`**. §68 should **not** assume a single await is cancellable mid-flight. Preferred approach:
   - Add optional **`cancel: &CancellationToken`** (or pass into a new **`yahoo_latest_quotes_for_symbols_cancellable`**) and check **`cancel.is_cancelled()`** between **`v7`** chunks and before spawning each fallback task.
   - On cancel: return **empty** quotes + no partial **`watchlist_quotes`** apply (batch-level drop per §68.4.3).
   - Keep **`run_stock_quote_batch`** as the orchestrator; avoid duplicating Yahoo chunk logic in **`app.rs`**.
...
#### 68.4.6 `deliver_fetch_done` and inflight when a task is cancelled

| Cancel outcome | Required behavior |
|----------------|-------------------|
| Task returns early with empty **`FetchDone::Stock`** (same **`generation`**) | **`apply_stock_fetch_done`** runs; if generation still current, clears inflight and may surface empty-quote UX — document whether empty batch is silent or status-only. |
| Task dropped without **`deliver_fetch_done`** | **`stock_refresh_inflight`** may stick **`true`** until **`InflightRecovery::Stock`** watchdog (**§39**) — **must** add test when implementing overlap. |
| Superseded generation + late **`FetchDone`** | Existing generation guard — no quote mutation (**§16.2.1**). |

**Policy (recommended):** Cancelled batches **always** call **`deliver_fetch_done`** with **`FetchDone::Stock { generation, quotes: empty, errors: [] }`** (or a single debug **`ApiMessage`** error) so inflight clears deterministically without relying on recovery.

#### 68.4.7 Implementation phases (engineer checklist)

| Phase | Deliverable | When |
|-------|-------------|------|
| **0 (default)** | §68 + QA doc-only; no **`tokio-util`**; single-flight unchanged | **Now** — no **§68.2** trigger |
| **1** | Unit test: stale **`generation`** does not mutate **`watchlist_quotes`** (may ship without overlap — satisfies Issue #191 partial AC) | Anytime before/without mode **B** |
| **2** | Product approves overlap + mode **B** spawn policy | Gate for HTTP cancel |
| **3** | **`tokio-util`**, **`stock_quote_cancel`**, **`run_stock_quote_batch`**, Yahoo cancellable helper, Polygon **`select!`** loop | Issue #191 code PR |
| **4** | §16.1 item 2 + §68.3 “when cancel needed” finalized; README note if **`allow_overlapping_quote_batches`** | Same PR as phase 3 |

### 68.5 Crate / module summary

| File | Change |
|------|--------|
| [`Cargo.toml`](../Cargo.toml) | **`tokio-util`** (when implementing mode **B**) |
| [`src/app/app.rs`](../src/app/app.rs) | **`stock_quote_cancel`**, **`spawn_stock_fetch_task`**, **`run_stock_quote_batch`**, **`apply_stock_fetch_done`** / recovery audit |
| [`src/api/yahoo.rs`](../src/api/yahoo.rs) | Cancellable chunk/fallback loop (§68.4.3 item 4) |
| [`src/app/fetch_delivery.rs`](../src/app/fetch_delivery.rs) | Regression tests if delivery policy changes |
| [`docs/SPEC.md`](SPEC.md) §16.1 item 2 | Cross-link §68 (**done**) |
| [`README.md`](../README.md) | Document overlap flag + debug delay for cancel QA (if mode **B**) |
...
| Test | Approach |
|------|----------|
| Superseded batch does not mutate quotes | Direct **`apply_stock_fetch_done(0, …)`** with **`stock_fetch_generation = 1`** (pattern exists near existing §16 tests ~L4213+); or **`#[tokio::test]`** harness: gen **N**, bump to **N+1**, apply **N** → **`watchlist_quotes`** unchanged |
| Stale gen leaves inflight when newer batch running | With overlap enabled: gen **N** in flight, bump to **N+1**, apply **N** → **`stock_refresh_inflight == true`** until **N+1** completes |
| Cancel mid-batch | **`STOCKTERM_DEBUG_HTTP_DELAY_MS`** at start of batch + cancel after spawn; Polygon: count **`get_quote`** via **`wiremock`**; Yahoo: assert no extra chunk/fallback after cancel between chunks |
| Cancelled task clears inflight | Cancel + empty **`FetchDone`** delivered → **`stock_refresh_inflight == false`** without user recovery |

Reuse existing §16 **`catch_unwind`** / **`InflightRecovery`** tests in [`src/app/app.rs`](../src/app/app.rs) and [`src/app/fetch_delivery.rs`](../src/app/fetch_delivery.rs) — regression only.
...
- [x] Maintainer approves §68 (phases 0–3 shipped 2026-05-30).
- [x] SPEC §16.1 / §68.3 document when cancel is vs is not required.
- [x] Superseded batch does not update **`watchlist_quotes`** (unit/integration test).
- [x] Cancel paths log at **`tracing::debug`** only.
- [x] **`cargo test`** + **`cargo clippy -- -D warnings`** green.
- [x] Manual QA Issue **#191** signed (2026-05-30).
- [x] No UI-thread blocking; TUI remains responsive under **`STOCKTERM_DEBUG_HTTP_DELAY_MS`**.
...
**Architect recommendation (2026-05-30):** Approve **§68 documentation** and **phase 0** (no code). **Defer phase 3** until a **§68.2** trigger is explicitly chosen (e.g. priority active-symbol refresh during a watchlist batch). Without that trigger, satisfy Issue #191’s “document when cancel is needed” via §16.1 + §68.3 only; optional **phase 1** stale-generation unit test may land without **`tokio-util`**.

After maintainer approval of **§68** **and** (for HTTP cancel) a **§68.2** product trigger, the **engineer** may implement per [`.cursor/rules/sdd_workflow.mdc`](../.cursor/rules/sdd_workflow.mdc) and run [`docs/QA_PLAN.md`](QA_PLAN.md) Issue **#191** before merge. If the trigger is **not** met, ship **doc-only** / **phase 1** tests only and leave [#191](https://github.com/FelipeMorandini/stockterm/issues/191) open per ROADMAP triage policy.

### 68.11 Shipment record
...
- **Follow-up:** [#214](https://github.com/FelipeMorandini/stockterm/issues/214) phase 2 — optional **`reqwest`** hard-abort beyond cooperative cancel (see **§74.10**; issue may stay open for that slice).
...
| **D** | In-app editor + live preview | **Shipped** — [#208](https://github.com/FelipeMorandini/stockterm/issues/208) (**§71**; **PR:** [#211](https://github.com/FelipeMorandini/stockterm/pull/211)) |
...
**Status:** **Shipped** (2026-05-27) — wizard modal editor on Dashboard tab (`e`), live preview, strict validation on commit, hot-reload caches. **Manual QA:** [`docs/QA_PLAN.md`](QA_PLAN.md) Issue **#208** (sign-off **2026-05-27**). **Code:** [`src/app/dashboard_editor.rs`](../src/app/dashboard_editor.rs).
...
## 74. Issue [#214](https://github.com/FelipeMorandini/stockterm/issues/214) — Overlap quote batches: inflight chain watchdog (§68 follow-up)

**Status:** **Shipped** (2026-05-30). Parent **§68** / [#191](https://github.com/FelipeMorandini/stockterm/issues/191) shipped **2026-05-30** (**PR:** [#215](https://github.com/FelipeMorandini/stockterm/pull/215)). Manual QA [`docs/QA_PLAN.md`](QA_PLAN.md) Issue **#214** sign-off **2026-05-30**.

**GitHub issue (synced 2026-05-30):** OPEN · label **`roadmap`** · title *Overlap quote batches: per-generation inflight watchdog when latest batch hangs*. Priority: deferred / low — only affects opt-in **`allow_overlapping_quote_batches: true`** when HTTP or channel delivery stalls.

**Sources:**

- [Issue #214](https://github.com/FelipeMorandini/stockterm/issues/214) — §68 overlap-mode gap: repeated supersede resets **`stock_inflight_since`**, so §39.2 watchdog never expires while status shows **Refreshing quotes…**.
- **§39.2** — **`recover_stale_inflight_flags`** + **`STOCKTERM_INFLIGHT_STALE_SECS`** (default **120**).
- **§68.4.6** — cancelled / dropped batches must not leave **`stock_refresh_inflight`** stuck **`true`** indefinitely.

### 74.1 Problem (verified in tree, 2026-05-30)

| Mode | `stock_inflight_since` reset | Watchdog behavior |
|------|------------------------------|-------------------|
| **Single-flight (default)** | Set once per batch at **`spawn_stock_fetch_task`** | §39.2 clears stuck inflight after **`INFLIGHT_STALE_AFTER`** — **works** |
| **Overlap (`allow_overlapping_quote_batches: true`)** | **Reset on every superseding spawn** (~L1538 in [`app.rs`](../src/app/app.rs)) | Timer restarts each immediate poll; user can keep **Refreshing quotes…** forever if each latest HTTP hang never completes |

**Failure scenario:**

1. Overlap enabled; user triggers **`request_immediate_stock_poll`** → generation **G**, **`stock_refresh_inflight = true`**, **`stock_inflight_since = T₀`**.
2. HTTP hangs (or **`FetchDone`** + **`InflightRecovery`** both fail to deliver).
3. User polls again before stale threshold → generation **G+1**, **`stock_inflight_since = T₁`** (supersede cancels prior token per §68).
4. Repeat every few seconds → **`stock_inflight_since`** always recent → **`recover_stale_inflight_flags`** never fires.

**Not a regression in default builds:** **`allow_overlapping_quote_batches`** defaults **`false`**; single-flight coalesces via **`stock_refresh_pending`** (**§16.3**).

### 74.2 Product goal

When overlap mode is enabled, the UI must **eventually** leave the stuck **Refreshing quotes…** state if quote-batch work does not complete within a bounded wall time — **even when** the user keeps issuing superseding immediate polls.

**Non-goals:**

- Changing default single-flight behavior.
- Replacing **`CancellationToken`** or **`stock_fetch_generation`** guards.
- Forcing overlap on for all users.
- **Phase 2 (optional follow-up):** hard-abort in-flight **`reqwest`** requests beyond cooperative cancel (Issue #214 “stronger abort” bullet) — defer unless profiling shows Yahoo **`v7`** chunk awaits ignore cancel for **> INFLIGHT_STALE_AFTER**.

### 74.3 Design — inflight **chain** timestamp (recommended)

Introduce a second timestamp that tracks the **start of the current unbroken inflight chain**, not the latest spawn:
...
| Event | `stock_inflight_since` | `stock_inflight_chain_since` |
|-------|------------------------|------------------------------|
| **`spawn_stock_fetch_task`** when **`!stock_refresh_inflight`** (chain begins) | `Some(now)` | `Some(now)` |
| **`spawn_stock_fetch_task`** when **`stock_refresh_inflight`** (overlap supersede) | `Some(now)` — reset | **unchanged** — keep chain start |
| **`apply_stock_fetch_done`** with matching generation (normal completion) | `None` | `None` |
| **`apply_inflight_recovery(Stock)`** / §39 watchdog clear | `None` | `None` |

**Watchdog selection** in **`recover_stale_inflight_flags`** (stock branch only):

```rust
let stale_reference = if self.config.allow_overlapping_quote_batches {
    self.stock_inflight_chain_since
} else {
    self.stock_inflight_since
};
if self.stock_refresh_inflight
    && Self::inflight_is_stale(stale_reference, stale_after)
{
    // overlap: also cancel active token so hung HTTP cooperatively exits
    if self.config.allow_overlapping_quote_batches {
        self.stock_quote_cancel.cancel();
    }
    tracing::warn!(/* domain = "stock", reason = "inflight chain stale" */);
    self.apply_inflight_recovery(InflightRecovery::Stock);
}
```

**Rationale:** Single-flight mode behavior is unchanged (chain start equals last spawn). Overlap mode measures **wall time stuck in inflight**, independent of supersede cadence.

**Rejected alternative:** Per-generation `(generation, deadline)` map — heavier state for rare opt-in mode; chain timestamp satisfies Issue #214 acceptance.

### 74.4 Recovery semantics (overlap)

When the chain watchdog fires:

1. **`stock_quote_cancel.cancel()`** — nudge cooperative cancel in **`run_stock_quote_batch`** / Yahoo chunk loop (**§68.4.3**).
2. **`apply_inflight_recovery(InflightRecovery::Stock)`** — clear **`stock_refresh_inflight`**, timestamps, and honor **`stock_refresh_pending`** respawn (**§16.3** / [#77](https://github.com/FelipeMorandini/stockterm/issues/77)).
3. **Do not** bump **`stock_fetch_generation`** on watchdog (late **`FetchDone`** from pre-recovery tasks remain stale via existing guard).
4. **Optional one-line status** (product minimum): reuse existing recovery path — **no new user-visible error string required** for v1; **`tracing::warn!`** to file log only.

Late **`FetchDone::Stock`** after recovery:

- **Stale generation** → ignored (**§16.2.1**).
- **Matching generation** empty cancel payload → clears inflight again (idempotent).

### 74.5 Config / env

| Knob | Change |
|------|--------|
| **`STOCKTERM_INFLIGHT_STALE_SECS`** | **Reuse** for chain watchdog (no new env for v1). Document overlap semantics in README §39 pointer. |
| **`allow_overlapping_quote_batches`** | Unchanged; §74 only active when **`true`**. |

Optional future: **`STOCKTERM_OVERLAP_INFLIGHT_CHAIN_SECS`** — only if maintainers want a shorter chain cap than channel-failure stale threshold. **Not v1.**

### 74.6 Crate / module summary

| File | Change |
|------|--------|
| [`src/app/app.rs`](../src/app/app.rs) | **`stock_inflight_chain_since`** field; set/clear in **`spawn_stock_fetch_task`**, **`apply_stock_fetch_done`**, **`apply_inflight_recovery(Stock)`**; extend **`recover_stale_inflight_flags`** stock branch per §74.3 |
| [`README.md`](../README.md) | One line: overlap mode uses **chain** timestamp for stale inflight recovery |
| [`docs/SPEC.md`](SPEC.md) §39.2 | Cross-link §74 for overlap **`stock_inflight_since`** nuance (doc-only in same PR) |

**No new dependencies.**

### 74.7 Automated verification
...
**Required tests (add `issue_214` filter or names containing `inflight_chain`):**

| Test | Approach |
|------|----------|
| Overlap chain stale despite recent supersede | **`allow_overlapping_quote_batches = true`**, **`stock_refresh_inflight = true`**, **`stock_inflight_chain_since = now - 11s`**, **`stock_inflight_since = now`**, **`STOCKTERM_INFLIGHT_STALE_SECS=10`** → **`recover_stale_inflight_flags`** clears inflight |
| Chain start on first spawn only | Two superseding spawns (mock or direct field mutation): **`chain_since`** unchanged, **`inflight_since`** updated |
| Normal completion clears chain | **`apply_stock_fetch_done`** matching generation → both timestamps **`None`** |
| Single-flight regression | Existing **`recover_stale_inflight_flags_clears_stock_inflight`** unchanged (overlap off → uses **`stock_inflight_since`**) |
| Recovery respawns pending | **`stock_refresh_pending = true`** + chain watchdog → **`apply_inflight_recovery`** schedules respawn (§16.3) |

### 74.8 Manual verification pointer

[`docs/QA_PLAN.md`](QA_PLAN.md) — **Issue #214** (overlap-only steps).

### 74.9 Acceptance criteria (Issue #214)

- [x] Maintainer approves **§74**.
- [x] Overlap mode: continuous supersede during hung HTTP cannot prevent chain watchdog from clearing **Refreshing quotes…** within **`STOCKTERM_INFLIGHT_STALE_SECS`**.
- [x] Default single-flight behavior unchanged (no config flag).
- [x] Watchdog logs **`tracing::warn!`** only (no **`println!`**).
- [x] **`cargo test`** + **`cargo clippy -- -D warnings`** green.
- [x] Manual QA Issue **#214** signed (2026-05-30).

### 74.10 Out of scope

- Historical / news / search / options inflight domains (unchanged §39.2).
- **`reqwest`** request abort / drop beyond existing **`CancellationToken`** (Issue #214 phase 2).
- UI toggle for overlap mode in Settings (remain JSON-only **`allow_overlapping_quote_batches`**).

### 74.11 Implementation sequence

1. Add **`stock_inflight_chain_since`** + lifecycle hooks in **`spawn_stock_fetch_task`** / apply paths.
2. Extend **`recover_stale_inflight_flags`** stock branch + overlap cancel call.
3. Unit tests (§74.7).
4. README + §39.2 cross-link.
5. Manual QA Issue **#214**.

### 74.12 Approval

After maintainer approval of **§74**, the **engineer** implemented per [`.cursor/rules/sdd_workflow.mdc`](../.cursor/rules/sdd_workflow.mdc). Manual QA [`docs/QA_PLAN.md`](QA_PLAN.md) Issue **#214** signed **2026-05-30**.
...
## 75. Issue [#216](https://github.com/FelipeMorandini/stockterm/issues/216) — Session-fresh / intraday watchlist quote feel (M2 remainder)

**Status:** **Shipped** (2026-06-01). Parent milestone: M2 **Real-time-ish quotes** ([`docs/ROADMAP.md`](ROADMAP.md) §6 item 3, §4.1).

**GitHub issue (synced 2026-06-01):** OPEN · label **`roadmap`** · title *M2: session-fresh / intraday watchlist quote feel*.

**Sources:**

- [Issue #216](https://github.com/FelipeMorandini/stockterm/issues/216) — watchlist quotes feel “latest session” but not intraday-fresh; improve via richer Yahoo session fields and/or **`v8`** intraday chart fallback. **Not** streaming / Level-2.
- **§17** — latest-session quotes via REST adapters into unchanged **`TickerResponse`** / **`TickerResult`** at UI boundaries.
- **§9.15** — Yahoo batched **`v7`** primary path; per-symbol **`yahoo_latest_quote`** fallback when batch misses.
- **§35** — **`refresh_rate`** throttle unchanged; this slice improves **what price** each poll returns, not poll cadence.

### 75.1 Problem (verified in tree, 2026-06-01)

| Symptom | Root cause (code) |
|---------|-------------------|
| Watchlist **Last** feels “stuck” between polls during regular hours | Primary Yahoo **`v7`** path maps **`regularMarketPrice`** only — acceptable when **`v7`** succeeds, but **change %** uses **`c − o`** (session open) instead of vs **previous close** ([`watchlist_display.rs`](../src/app/watchlist_display.rs), [`ui.rs`](../src/app/ui.rs) **`draw_stock_detail`**) — diverges from typical market UX. |
| Fallback quotes especially stale | **`yahoo_quote_at`** uses **`v8/finance/chart?range=1d&interval=1d`** ([`v8_chart_latest_url`](../src/api/yahoo.rs)) — one **daily** bar; **`chart_to_ticker`** reads **meta** only, so **`c`** / **`t`** do not track intraday bar updates. |
| Extended hours ignored | **`V7QuoteItem`** omits **`postMarketPrice`**, **`preMarketPrice`**, **`marketState`** despite **§9.9** mentioning **`postMarketPrice`** open fallback — after-hours **`regularMarketPrice`** can lag. |
| Polygon users see prior session close | **`PolygonProvider::get_quote`** — daily aggregates only (**§17.4**); free tier — **document**, do not “fix” in #216. |

**Non-problem (do not regress):** Batched **`v7`** HTTP reduction (**§9.15**), single-flight / overlap cancel (**§68** / **§74**), **`MarketDataProvider`** trait shape, alert **`get_current_price`** lookup paths.

### 75.2 Product goal

During a normal Yahoo-backed session, watchlist and Stock View detail should show:

1. **Last** — the best available **session-aware** price (regular / pre / post per Yahoo **`marketState`**).
2. **Change / %** — vs **previous close** when the provider supplies it (Yahoo **`regularMarketPreviousClose`** or **`regularMarketChange`**), not vs session open only.
3. **Fallback resilience** — when **`v7`** is empty or fails, **`v8`** quote fallback uses **1-minute** intraday bars so **`c`** / **`t`** reflect the latest minute close, not a single end-of-day bar.

**Non-goals:**

- WebSocket / true streaming / Level-2 book.
- Polygon paid-tier snapshot / intraday quote endpoints.
- New Settings row or **`Config`** field for v1 (always-on for Yahoo).
- Watchlist **“as of HH:MM”** column or market-hours badge (optional follow-up).
- Changing **`Config.refresh_rate`** defaults or UI tick interval.

### 75.3 Design — Yahoo **`v7`** session-aware mapping

**Files:** [`src/api/yahoo.rs`](../src/api/yahoo.rs).

#### 75.3.1 Extend private **`V7QuoteItem`**

Add **`#[serde(default)]`** optional fields (camelCase):

| Field | Purpose |
|-------|---------|
| **`marketState`** | `"REGULAR"`, `"PRE"`, `"PREPRE"`, `"POST"`, `"POSTPOST"`, `"CLOSED"`, … |
| **`preMarketPrice`**, **`preMarketTime`** | Pre-market display price / Unix **seconds** |
| **`postMarketPrice`**, **`postMarketTime`** | Post-market display price / Unix **seconds** |
| **`regularMarketChange`**, **`regularMarketChangePercent`** | Optional cross-check (prefer computing from mapped **`c`** + **`prev_close`**) |

#### 75.3.2 Session price selection

Add **`fn yahoo_v7_session_price(q: &V7QuoteItem) -> ProviderResult<(f64, i64)>`** returning **`(close, time_sec)`**:

| **`marketState`** (case-insensitive prefix) | **`c`** source | **`t`** source |
|---------------------------------------------|----------------|----------------|
| **`PRE`**, **`PREPRE`** | **`preMarketPrice`** → **`regularMarketPrice`** | **`preMarketTime`** → **`regularMarketTime`** → **`Utc::now()`** |
| **`POST`**, **`POSTPOST`** | **`postMarketPrice`** → **`regularMarketPrice`** | **`postMarketTime`** → **`regularMarketTime`** → **`Utc::now()`** |
| **`REGULAR`**, **`CLOSED`**, empty / unknown | **`regularMarketPrice`** (required) | **`regularMarketTime`** → **`Utc::now()`** |

Document precedence in **`///`** on the selector; unit-test each branch with fixture JSON.

#### 75.3.3 **`v7_item_to_ticker_response`**

- Call session selector for **`c`** / **`t`** (ms via **`normalize_bar_timestamp_to_ms`**).
- Keep **`o`** = **`regularMarketOpen`** with fallbacks unchanged (**§17.3**).
- Keep **`h`** / **`l`** / **`v`** from **`regularMarketDayHigh`** / **`regularMarketDayLow`** / volume.
- Populate new **`TickerResult::prev_close`** from **`regularMarketPreviousClose`** when present.

Batch path (**§9.15**) reuses this mapper — no duplicate logic.

### 75.4 Design — **`TickerResult`** change baseline (minimal model extension)

**File:** [`src/models/ticker.rs`](../src/models/ticker.rs).

Add optional field (backward-compatible with Polygon JSON):

```rust
/// Previous session close for change % when provider supplies it (Issue #216 / §75).
#[serde(default)]
...
**Polygon adapter:** when **`get_quote`** returns **≥ 2** daily bars (**`sort=desc`**), set **`prev_close`** on the latest bar from the **second** bar’s **`c`**; when only one bar, leave **`None`** (UI falls back to **`o`**).

**UI / display cache** — update change math in:

| File | Change |
|------|--------|
| [`src/app/watchlist_display.rs`](../src/app/watchlist_display.rs) | **`quote_cells_for_symbol`**: **`price_change = bar.c - bar.change_reference()`** |
| [`src/app/ui.rs`](../src/app/ui.rs) **`draw_stock_detail_in`** | Same for detail pane **Change** line; **Open:** still shows **`result.o`** |

**Alerts:** **`get_current_price`** continues to use **`latest_result().c`** — session-aware **`c`** applies automatically; threshold math unchanged.

### 75.5 Design — Yahoo **`v8`** intraday quote fallback

**Scope:** quote orchestration only (**`yahoo_quote_at`** / **`yahoo_latest_quote_orchestrate`** fallback branch). **Do not** change **`yahoo_historical_range`** or Charts historical paths.

1. Replace **`v8_chart_latest_url`** interval **`1d`** → **`1m`** for the **quote-fallback** URL builder (rename to **`v8_chart_intraday_latest_url`** or add **`interval`** param — quote path passes **`"1m"`**).
2. Extend **`chart_to_ticker`** (or add **`chart_to_ticker_from_intraday`**) to:
   - Read session OHLC from **`ChartMeta`** as today.
   - When **`timestamp`** + **`indicators.quote[0].close`** exist, set **`c`** and **`t`** from the **last non-null** minute bar (reuse / share logic with **`last_close_from_bars`**, but return timestamp too).
   - When bars empty, keep meta-only mapping (same as today).
   - Set **`prev_close`** from **`meta.chart_previous_close`** when present.
3. **`wiremock`** test (**§32** / Issue **#89**): assert fallback GET URL contains **`interval=1m`** (update fixture if URL matcher is exact).

**Rate / payload:** Fallback runs only for **`v7`** misses per symbol (**§9.15.4**), bounded by existing **`Semaphore`** — acceptable vs daily fallback. Respect **`CancellationToken`** in batch fallback **`JoinSet`** (unchanged).

### 75.6 Application layer

**No change** to **`run_stock_quote_batch`**, **`spawn_stock_fetch_task`**, **`apply_stock_fetch_done`**, or **`FetchDone::Stock`** shape — all freshness work stays in **`api/`** + display helpers.

**Rebuild triggers:** Existing **`rebuild_watchlist_display_rows`** after quote apply remains sufficient.

### 75.7 Crate / module summary

| File | Change |
|------|--------|
| [`src/api/yahoo.rs`](../src/api/yahoo.rs) | **`V7QuoteItem`** extended fields; **`yahoo_v7_session_price`**; **`v7_item_to_ticker_response`** session mapping; **`v8`** quote URL **`interval=1m`**; **`chart_to_ticker`** last-minute bar for **`c`**/**`t`**; unit + **`wiremock`** tests |
| [`src/models/ticker.rs`](../src/models/ticker.rs) | **`prev_close`**, **`change_reference()`**; tests |
| [`src/api/polygon.rs`](../src/api/polygon.rs) | Populate **`prev_close`** from second bar when available; doc comment |
| [`src/app/watchlist_display.rs`](../src/app/watchlist_display.rs) | Change baseline via **`change_reference()`** |
| [`src/app/ui.rs`](../src/app/ui.rs) | Detail pane change line via **`change_reference()`** |
| [`docs/QA_PLAN.md`](QA_PLAN.md) | Issue **#216** manual steps |
| [`README.md`](../README.md) | One line under quotes: Yahoo session-aware pricing; Polygon daily-bar limitation |

**No new dependencies.**

### 75.8 Automated verification
...
cargo test yahoo_latest_quote   # §32 wiremock — update interval expectation
cargo clippy -- -D warnings
```
...
| Test | Assert |
|------|--------|
| **`v7_maps_post_market_price_when_market_state_post`** | **`marketState: "POST"`** → **`c`** from **`postMarketPrice`** |
| **`v7_maps_pre_market_price`** | **`PRE`** branch |
| **`v7_prev_close_populates_ticker_result`** | **`regularMarketPreviousClose`** → **`prev_close`** |
| **`change_reference_falls_back_to_open`** | **`prev_close: None`** → **`change_reference() == o`** |
| **`chart_to_ticker_uses_last_minute_bar_close`** | Fixture with 1m bars → **`c`** / **`t`** from last bar |
| **`chart_to_ticker_meta_only_when_no_bars`** | Empty bars → meta mapping unchanged |
| **`wiremock_yahoo_v8_fallback_uses_1m_interval`** | Fallback GET matches **`interval=1m`** |

### 75.9 Manual verification pointer

[`docs/QA_PLAN.md`](QA_PLAN.md) — **Issue #216**.

### 75.10 Acceptance criteria (Issue #216)

- [x] Maintainer approves **§75**.
- [x] Yahoo **`v7`** (single + batch): extended-hours **`marketState`** selects correct display price.
- [x] Watchlist **Chg** / **%** and Stock View **Change** use **previous close** when Yahoo supplies **`regularMarketPreviousClose`**.
- [x] Yahoo **`v7`→`v8`** fallback requests **`interval=1m`** and maps last minute bar when present.
- [x] Polygon path unchanged except optional **`prev_close`** from prior daily bar; README documents daily-bar limit.
- [x] **`TickerResponse`** / batch / alert code paths unchanged at trait boundaries.
- [x] **`cargo test`** + **`cargo clippy -- -D warnings`** green.
- [x] Manual QA Issue **#216** signed (**2026-06-01**).

### 75.11 Out of scope

- Polygon intraday / snapshot quotes without paid plan.
- WebSocket streaming.
- Settings toggle for “session fresh” mode.
- **`v8`** multi-symbol quote batching (deferred since **§17.7**).
- Quote-age column / market-hours indicator in TUI.

### 75.12 Implementation sequence

1. **`TickerResult::prev_close`** + **`change_reference()`** + model tests.
2. Yahoo **`V7QuoteItem`** + **`yahoo_v7_session_price`** + **`v7_item_to_ticker_response`** + unit fixtures.
3. **`v8`** intraday fallback URL + **`chart_to_ticker`** minute-bar logic + tests.
4. UI change math (**watchlist_display**, **draw_stock_detail_in**).
5. Polygon **`prev_close`** from second bar (optional polish in same PR).
6. Update **§32** **`wiremock`** test for **`interval=1m`**.
7. README note; manual QA Issue **#216**.

### 75.13 Approval

After maintainer approval of **§75**, the **engineer** implemented per [`.cursor/rules/sdd_workflow.mdc`](../.cursor/rules/sdd_workflow.mdc). Manual QA [`docs/QA_PLAN.md`](QA_PLAN.md) Issue **#216** signed **2026-06-01**.

### 75.14 Shipment record

- **Date:** 2026-06-01.
- **PR:** [#218](https://github.com/FelipeMorandini/stockterm/pull/218).
- **Tracking:** [Issue #216](https://github.com/FelipeMorandini/stockterm/issues/216).
- **Code:** [`src/api/yahoo.rs`](../src/api/yahoo.rs) (session **`v7`** mapping, **`v8`** `interval=1m` fallback, max-timestamp minute bars, finite price guard); [`src/models/ticker.rs`](../src/models/ticker.rs) (`prev_close`, `change_reference`); [`src/api/polygon.rs`](../src/api/polygon.rs) (`enrich_polygon_quote_prev_close`); [`src/app/watchlist_display.rs`](../src/app/watchlist_display.rs), [`src/app/ui.rs`](../src/app/ui.rs); [`README.md`](../README.md).
- **Automated:** `cargo test issue_216`, `cargo clippy -- -D warnings`.
- **Manual QA:** [`docs/QA_PLAN.md`](QA_PLAN.md) Issue **#216** — sign-off **2026-06-01**.